...
class Reader(BaseModel):
    ...
    def parse_profiles(self, xml_files: list[str | IO], start_dict: dict | None = None):
        ...
        :param xml_files:   List of the path to the profiles (string) or file-like object
    ...
    def _instantiate_classes(self, xml_file: str | IO, bases: tuple, namespace_rdf: str):
        """creates/updates the python objects with the information of xml_file

        :param xml_file:        Path to the profile (string) or file-like object
        ...
        if not isinstance(xml_file, str):
            xml_file.seek(0)
        context = etree.iterparse(xml_file, ("start", "end"), remove_blank_text=True)
        level = 0
        ...
        for event, elem in context:
            ...
            class_namespace = next(
                (namespace for namespace in bases if str(elem.tag).startswith(namespace)),
                None,
            )
            if event == "end" and class_namespace is not None and level == 1:
    ...
    def _extract_classname_uuid(elem, class_namespace: str, namespace_rdf: str) -> tuple:
        ...
        :return:    class_name, uuid  (example "ACLineSegment", instance_uuid: "mRID")
        """
