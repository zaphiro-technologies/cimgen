...
class Base:
    ...
    def recommended_profile(self) -> BaseProfile:
        ...
        raise NotImplementedError(
            "Method not implemented because not relevant in Base.",
        )
    ...
    def get_attribute_main_profile(self, attr: str) -> BaseProfile | None:
        ...
        if self._is_included_in_profile(profiles, self.recommended_profile):
            return self.recommended_profile
        if profiles:
            return sorted(profiles)[0]
        return None

    def cgmes_attribute_names_in_profile(
        self,
        ...
        return {
            ...
            if profile is None
            or self._is_included_in_profile(
                Base.get_extra_prop(f, "in_profiles"),
                profile,
            )
            if Base.get_extra_prop(f, "is_used")
        }

    def cgmes_attributes_in_profile(
        self,
        profile: BaseProfile | None,
    ) -> dict[str, "CgmesAttribute"]:
        """
        ...
        for parent in reversed(self.__class__.__mro__[:-1]):
            for f in fields(parent):
                ...
                if (
                    f
                    not in self.cgmes_attribute_names_in_profile(
                        profile,
                    )
                    or shortname in seen_attrs
                ):
                    continue
                ...
                if extra and extra.get("is_used"):
                    ...
                    extra_info = {
                        ...
                        "attribute_main_profile": self.get_attribute_main_profile(
                            shortname,
                        ),
                    }
    ...
    def to_xml(
        ...
        if (
            profile_attributes == {}
            or obj_id is None
            or not self._is_included_in_profile(
            ...
            if self.apparent_name().lower() != "terminal":
                obj_id = "_" + obj_id
            # Create root element
            nsmap = NAMESPACES
            root = etree.Element(
                "{" + self.namespace + "}" + self.resource_name,
                nsmap=nsmap,
            )

            # Add the ID as attribute to the root
            if self._is_included_in_profile(
                [self.recommended_profile],
                profile_to_export,
            ):
            ...
            root = self._add_attribute_to_etree(
    ...
    def _get_attributes_to_export(
        self,
        profile_to_export: BaseProfile,
    ) -> dict:
        attributes_to_export = {}
        attributes_in_profile = self.cgmes_attributes_in_profile(
            profile_to_export,
        )
        for key, attribute in attributes_in_profile.items():
            if not self._is_included_in_profile(
                self.possible_profiles, attribute["attribute_main_profile"]
            ) or self._is_included_in_profile(
                [attribute["attribute_main_profile"]],
                profile_to_export,
            ):
                attributes_to_export[key] = attribute
        attributes_to_export = self._remove_empty_attributes(
            attributes_to_export,
        )
    ...
    def _remove_empty_attributes(attributes: dict) -> dict:
        for key, attribute in list(attributes.items()):
            ...
            if attribute["value"] in [None, "", []]:
                ...
                and attribute["attribute_class"] == "Boolean"
            ):
                attribute["value"] = str(attribute["value"]).lower()
        return attributes

    @staticmethod
    def _is_included_in_profile(
        possible_profiles: list[BaseProfile] | set[BaseProfile],
        profile_to_export: BaseProfile,
    ) -> bool:
        for possible_profile in possible_profiles:
            if all(uri in profile_to_export.uris for uri in possible_profile.uris):
                return True
        return False

    @staticmethod
    def _add_attribute_to_etree(
        attributes: dict,
        root: etree._Element,
        nsmap: dict,
    ) -> etree._Element:
        ...
        for field_name, attribute in attributes.items():
            ...
            if attribute["is_class_attribute"]:
                ...
                if ".terminal" in element_name.lower():
                    element.set(
                        rdf_namespace + "resource",
                        "#" + attribute["value"],
                    )
                else:
                    element.set(
    ...
    def _parse_xml_fragment(self, xml_fragment: str) -> dict:
        ...
        for key, class_attribute in class_attributes.items():
            ...
            attr_value = self._extract_attr_value_from_etree(
                attr,
                class_attribute,
                xml_attributes,
            )
    ...
    def _extract_attr_value_from_etree(
        self,
        attr_name: str,
        class_attribute: "CgmesAttribute",
        xml_attributes: list[etree._Element],
    ):
